...
public function dislike($ref, $playlist_id, $utilisateur_id)
{
    ...
    private function vote($ref, $playlist_id, $utilisateur_id, $vote)
    {
        ...
        $req = $this->pdo->prepare("SELECT id, vote FROM votes WHERE ref = ? AND playlist_id = ? AND utilisateur_id = ?");
        ...
        if ($vote_row) {
            ...
            $this->pdo->prepare("UPDATE votes SET vote = ?, created_at = ? WHERE id = {$vote_row->id}")->execute([$vote, date('d-m-Y H:i')]);
            return true;
        }
        $req = $this->pdo->prepare("INSERT INTO votes SET ref = ?, playlist_id = ?, utilisateur_id = ?, created_at = ?, vote = $vote");
        $req->execute([$ref, $playlist_id, $utilisateur_id, date('d-m-Y H:i')]);
    }

    public function updatecount($ref, $playlist_id)
    {
        $req = $this->pdo->prepare("SELECT COUNT(id) as count, vote FROM votes WHERE ref = ? AND playlist_id = ? GROUP BY vote");
        ...
        $req = $this->pdo->query("UPDATE $ref SET like_count = {count [1]}, dislikecount = {$counts[-1]} WHERE id = $ref_id");
        return true;
    }
